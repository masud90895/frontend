...
function getStyleLoaders(isProd, sourceMap = false) {
    return [
        isProd
            ? {
...
function createWebpackConfig() {
    ...
    if (!CI)
        console.table(Object.keys(includeEnv)
    ...
    const generateWebManifest = new GenerateJsonPlugin({
        ...
        value: {
            ...
            description: process.env.MFOX_PWA_APP_DESCRIPTION ||
    ...
    return {
        ...
        output: {
            filename: isProd || process.env.MFOX_BUNDLE_ANALYZER
                ? 'static/js/[name].[contenthash:8].js'
                : 'static/js/bundle.[contenthash:8].js',
            chunkFilename: isProd && !process.env.MFOX_BUNDLE_ANALYZER
